...
function SalesPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("sales")
        .select(
          "id, receipt_number, total, sale_type, payment_method, lipa_namba_provider, status, created_at, cashier_id, customer_id, customer:customers(name)",
        )
  ...
  const filtered = (data ?? []).filter(
    (s) => !q || s.receipt_number.toLowerCase().includes(q.toLowerCase()),
  );
      ...
      <div className="card-elev divide-y divide-border">
        {filtered.length === 0 && (
          <div className="p-12 text-center">
            <ReceiptIcon className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
            <p className="text-sm text-muted-foreground">Hakuna mauzo.</p>
          </div>
        )}
        {filtered.map((s) => (
          <Link
          ...
          >
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                ...
                {s.sale_type === "credit"
                  ? `Deni · ${s.customer?.name ?? "Mteja"}`
                  : s.payment_method === "lipa_namba"
                    ? `Lipa Namba (${s.lipa_namba_provider ?? "?"})`
                    : "Cash"}
                {s.status === "voided" && " · IMEFUTWA"}
              </div>
            </div>
            <div
              className={`font-semibold ${s.status === "voided" ? "line-through text-muted-foreground" : ""}`}
            >
              {formatMoney(s.total)}
            </div>
          </Link>
