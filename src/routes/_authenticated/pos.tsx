...
type PendingSale = {
  ...
  payment_method: "cash" | "credit"; // user choice
  cashier_id: string;
  shop_id: string;
...
function PosPage() {
    ...
    const syncOffline = async () => {
      ...
      try {
        for (const sale of queue) {
          // In offline sync, we need to translate payment_method to sale_type
          const saleType = sale.payment_method === "credit" ? "credit" : "cash";
          const { error: saleError } = await supabase.from("sales").insert({
            ...
            total: sale.total,
            payment_method: "cash", // DB always expects 'cash' or 'lipa_namba'
            sale_type: saleType,
            status: "completed",
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const saleType = paymentMethod === "credit" ? "credit" : "cash";
      ...
      if (isOnline) {
        const { error: saleError } = await supabase.from("sales").insert({
          ...
          discount,
          total,
          payment_method: "cash", // Always 'cash' for DB; use sale_type for credit/cash distinction
          sale_type: saleType,
          status: "completed",
      ...
      // If the error was already shown via toast, skip duplicate
