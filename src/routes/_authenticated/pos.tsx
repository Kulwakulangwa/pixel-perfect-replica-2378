...
import { Badge } from "@/components/ui/badge";import { createFileRoute } from "@tanstack/react-router";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import { useState, useEffect, useRef, useCallback } from "react";
import { toast } from "sonner";
...
function PosPage() {
  ...
  // --- Fetch products (using pos_products function) ---
  const { data: products = [], isLoading: productsLoading } = useQuery({
    queryKey: ["pos-products"],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("pos_products");
      ...
      try {
        for (const sale of queue) {
          ...
          for (const item of sale.items) {
            ...
            if (itemError) {
              console.error("Item sync error", itemError);
              // We might want to rollback? For simplicity, we'll skip.
            }
          }
          // Remove from queue
          await removeFromOfflineQueue(sale.id);
          toast.success(`Sale ${sale.id.slice(0,8)} synced!`);
        }
        setOfflineCount(0);
        queryClient.invalidateQueries({ queryKey: ["pos-products"] });
      } catch (e) {
      ...
      <Dialog open onOpenChange={onClose}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 font-mono text-sm">
            <div className="text-center border-b pb-2">
              <div className="font-bold text-lg">Wakuja Shop</div>
              <div className="text-xs text-muted-foreground">Risiti #{sale.id.slice(0,8)}</div>
              <div className="text-xs text-muted-foreground">
                {format(new Date(sale.created_at), "dd/MM/yyyy HH:mm")}
              </div>
            </div>
